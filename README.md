...
This project delivers a sophisticated, end-to-end solution to automate and optimise the Amazon FBA (Fulfilled by Amazon) wholesale business. By intelligently processing supplier data, enriching it with real-time market insights from leading e-commerce APIs, and performing advanced profitability calculations, `wholesaleFBA` empowers informed purchasing decisions and maximises return on investment.

## Key Capabilities

*   **Automated Data Processing:** Efficiently handles large supplier lists (100,000+ entries) using a robust Google Sheets-based VBA macro for initial cleansing and structuring.
*   **Comprehensive Data Enrichment:** Integrates with Amazon SP-API, Keepa API, and Jungle Scout API to gather critical real-time and historical market data.
...
*   **Modular and Scalable Architecture:** Designed with clear separation of concerns, facilitating maintenance, future enhancements, and adaptability to evolving e-commerce landscapes.

## End-to-End Workflow: A Visual Overview
...
1.  **Supplier Data Ingestion & Initial Cleansing (Google Sheets & VBA)**
    *   **Data Receipt:** Raw supplier data (CSV or Excel) is received.
    *   **Conceptual Email Automation:** (Due to signed NDA, actual code cannot be provided. This is a conceptual outline.) An automated process would ideally retrieve supplier files from a dedicated email inbox and pass them to the Google Sheets API for initial processing.
...
2.  **Python Processing - Data Enrichment & API Integration (Python Script)**
    ...
    *   **Keepa API (Barcode to ASIN):** For each product, the script uses the Keepa API to convert the supplier's EAN barcode into an Amazon Standard Identification Number (ASIN), crucial for subsequent Amazon API calls.
    *   **Amazon SP-API (Catalog Items):** The script queries the Amazon Selling Partner API (specifically the Catalog Items API for the UK marketplace) to retrieve real-time product details, including ASIN (if not already obtained from Keepa), Product Title, Current Buy Box Price, Sales Rank, and Main Product Image URL.
    *   **Amazon SP-API (Product Fees):** The script then uses the Amazon SP-API's Product Fees API to obtain estimated FBA (Fulfilled by Amazon) fees and Referral fees associated with the product.
    *   **Keepa API (Historical Data & Competitive Sellers):** The script leverages the Keepa API to fetch comprehensive historical data, including historical price fluctuations, sales rank trends, detailed Buy Box history (utilised by the VBA macro for competitive seller analysis), and estimated sales velocity.
    *   **Jungle Scout API:** The script integrates with the Jungle Scout API to acquire critical market intelligence, such as estimated monthly sales volumes, competitive landscape analysis, and product opportunity scores.
...
3.  **VBA Macro - Final Analysis & Reporting (within Google Sheets)**
